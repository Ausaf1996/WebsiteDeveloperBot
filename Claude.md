...
WhatsApp bot that updates the PAN Medical & Industrial Supplies website via Claude API.
Non-technical users send plain-English requests on WhatsApp; the bot applies changes to the live site.
...
Cloudflare Worker  (src/worker.py)
      │
      ├─► WhatsApp API  (src/whatsapp.py)   — parse incoming, send replies
...
└── src/
    ...
    ├── bot.py              # Core logic: confirmation flow, message routing
    ...
    └── whatsapp.py         # WhatsApp Business API — webhook verify, parse, send
```
...
2. Checks KV for a pending confirmation for this phone number → handles YES/NO.
...
Manages three KV key patterns per phone number:
- `pending:{phone}` — pending HTML + summary awaiting YES/NO (1-hour TTL).
- `rollback:{phone}` — snapshot of the HTML before the last applied change (24-hour TTL).
- `history:{phone}` — last 20 conversation messages for Claude context (24-hour TTL).
...
### src/whatsapp.py
- `parse_incoming_message()` — extracts phone number and text from webhook payload.
- `send_message()` — sends a text reply via the WhatsApp Business API.
- `verify_webhook()` — handles the hub.challenge verification handshake.
...
|---|---|
| `WHATSAPP_TOKEN` | WhatsApp Business API bearer token |
...
- **Conversation history** — last 20 messages per phone number are stored in KV and sent to Claude as multi-turn context, enabling natural follow-up requests.
